...
#ifndef SILSIM_ZOEM8Q0_H
#define SILSIM_ZOEM8Q0_H
...
#define ZOEM8Q0_WHOAMI_REG 0xA2
     ...
     * float std_lon          :     Standatd for longitude (in degrees +
     * minutes) uint8_t lon_direction  :     Direction for longitude (East or
     * West) float height           :     Current height above sea level in m
     */
    struct NMEA_DATA {
        float std_lat;
...
#define END_SEQ 0x0D0A
    ...
    /* Converts the latitude and longitude into degree format and sets the
     * global variables.*/
    void convert_data();

    /*
     *Finds lattitude, longitude and altitude using NMEA protocol with GPRMC and
     *xxGGA sentences. Returns true if there is a signal position lock and false
     *otherwise.
     *
     * EXAMPLE:
     * "$GPGGA,16405.00,4005.42330,N,10511.10842,W,1,09,0.91,15840,M,-21.4,M,1.0,0000*53"
     *
     * 2nd comma signals latitude is next (in degree + minute format) so
     *4005.42330 N is really 40 degrees + 05.42330/60 degrees = 45.09038833 N
     * 3rd comma signals N/S is next
     * 4th comma signals longitude is next (in degree + minute format) so
     *10511.10842 N is really 105 degrees + 11.10842/60 degrees = 105.1851403 W
     * 5th comma signals E/W
     * 6th comma signals the position fix quality
     *      0 = No fix, 1 = Autonomous GNSS fix, 2 = Differential GNSS fix, 4
     *=RTK fixed, 5 = RTK float, 6 = Estimated/Dead reckoning fix 9th comma
     *signals the altitude in m is next
     */
    bool process_GPS_NMEA(uint16_t CS_pin);

    /* Helper function in decode_xxGGa_sentence. Creates a substring within the
     * char array. Returns a char pointer to the new substring
     */
    char* arr_substring(uint8_t* buffer, int start, int end);

    /*
     *Decodes the current xxgga_sentence separated by commas and fills
     *lattitude, longitude, and altitude. Returns true if there is a signal
     *position lock and false otherwise.
     */
    bool decode_xxgga_sentence(uint8_t* buffer, uint16_t dataLength);
};

#endif  // SILSIM_ZOEM8Q0_H
